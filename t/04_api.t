...
SKIP: {
    ...
    my $http = EPPlication::Client::REST->new(
        host => $host,
        port => $port,
        ua_options => {
...

    {
        ...
        my $res = $http->request( 'POST', "/api/login", undef, $content_raw );
        is( $res->code, 200, 'login response status 200' );
    }

    {
        my $res = $http->request( 'GET', "/api/user" );
        is( $res->code, 200, 'get user list via api, response status 200' );
        my $users = decode_json($res->decoded_content(charset => 'none'));
...

    {
        ...
        my $res = $http->request( 'GET', "/api/test?tags=all&branch_id=$branch_id" );
        is( $res->code, 200, 'get tests via api, response status 200' );
        my $tests = decode_json($res->decoded_content(charset => 'none'));
        is(scalar @$tests, 1, '1 test found.');
        is($tests->[0]->{name}, 'some_test_name', 'correct test name');

        {
            sub get_jobs {
                my $res = $http->request( 'GET', "/api/job?filter=all" );
                is( $res->code, 200, 'get jobs via api, response status 200' );
                my $jobs = decode_json($res->decoded_content(charset => 'none'));
                return $jobs;
            }
            my $jobs = get_jobs();
            ...
            my $res = $http->request( 'POST', "/api/job", undef, $content_raw );
            is( $res->code, 201, 'response status 201' );
            $jobs = get_jobs();
            is(scalar @$jobs, 1, 'one job exists.');

            # delete job
            $res = $http->request( 'DELETE', "/api/job/" . $jobs->[0]->{id} );
            $jobs = get_jobs();
            is(scalar @$jobs, 0, 'no jobs exist.');
        }

        $res = $http->request( 'DELETE', "/api/test/" . $test->id );
...

    {
        ...
        ok( $tag, 'tag created' );
        my $res = $http->request( 'GET', "/api/tag" );
        is( $res->code, 200, 'response status 200' );
        my $tags = decode_json($res->decoded_content(charset => 'none'));
