...
use warnings;
use 5.010;
use EPPlication::Util::SchemaUpgradeHelper qw/ get_params add_param set_params /;
...
schema_from_schema_loader(
    ...
    sub {
        ...
        $step_rs = $schema->resultset('Step')->search( { type => 'REST' } );
        while ( my $step = $step_rs->next ) {
            say "Upgrade for step " . $step->id;
            my $params = get_params($step);
            add_param( $params, 'headers', '[% rest_headers_default %]' );
