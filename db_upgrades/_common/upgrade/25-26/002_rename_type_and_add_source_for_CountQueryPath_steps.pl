...
# schema upgrade v25-v26
# change type from CountPath[EPP|SOAP|REST] to CountQueryPath
...
schema_from_schema_loader(
    ...
    sub {
        ...
        say "Rename 'CountPath[EPP|SOAP|REST]' to 'CountQueryPath'\n"
            . "add parameter key 'source' with value [epp|soap|rest]\n";

        while ( my $step = $steps_rs->next ) {
            if ($step->type eq 'CountPathEPP') {
                update_with_source($step, '_epp_response');
            }
            elsif ($step->type eq 'CountPathSOAP') {
                update_with_source($step, '_soap_response');
            }
            elsif ($step->type eq 'CountPathREST') {
